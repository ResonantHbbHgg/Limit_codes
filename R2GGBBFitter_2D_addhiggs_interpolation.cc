...
// Francois Lagarde
std::map<std::string,std::vector<double>>Coeffs
{
{"mgg_sig_m0_cat0",{124.6,0.0004831}},{"mgg_sig_sigma_cat0",{2.054,-0.0008489}},{"mgg_sig_alpha_cat0",{0.9752,0.0003221}},{"mgg_sig_n_cat0",{4.541,0.001198}},{"mgg_sig_gsigma_cat0",{1.24,-0.0009692}},{"mgg_sig_frac_cat0",{0.5,0.0}},
{"mgg_sig_m0_cat1",{124.7,0.0003534}},{"mgg_sig_sigma_cat1",{1.802,0.0001981}},{"mgg_sig_alpha_cat1",{0.9045,7.303e-5}},{"mgg_sig_n_cat1",{8.85,0.002724}},{"mgg_sig_gsigma_cat1",{1.348,-0.001238}},{"mgg_sig_frac_cat1",{0.5,0.0}},
{"mjj_sig_m0_cat0",{110.4,0.03531}},{"mjj_sig_sigma_cat0",{18.42,-0.01996}},{"mjj_sig_alpha_cat0",{3.883,-0.005749}},{"mjj_sig_n_cat0",{4.232,0.001021}},{"mjj_sig_gsigma_cat0",{24.35,-0.007547}},{"mjj_sig_frac_cat0",{0.3,0.0}},
{"mjj_sig_m0_cat1",{104.5,0.0472}},{"mjj_sig_sigma_cat1",{23.53,-0.03022}},{"mjj_sig_alpha_cat1",{6.883,-0.01416}},{"mjj_sig_n_cat1",{4.708,0.01317}},{"mjj_sig_gsigma_cat1",{44.32,-0.03392}},{"mjj_sig_frac_cat1",{0.3,0.0}},
{"High_medium",{13.6941,-0.0160487,9.02653e-5}},{"High",{10.1044,-0.028701,7.63524e-5}}

};
std::vector<Int_t>Known;
std::vector<Int_t>Generated;

bool IsKnown(int sigMass)
{
...
{
        if(Coeffs.find(name)!=Coeffs.end())
	{
        	if(name=="High") return 0.198*(Coeffs[name][0]+Coeffs[name][1]*sigMass+Coeffs[name][2]*sigMass*sigMass);
		else if(name=="High_medium") return 0.198*(Coeffs[name][0]+Coeffs[name][1]*sigMass+Coeffs[name][2]*sigMass*sigMass-CalculateValue("High",sigMass)/0.198);
        	else return Coeffs[name][0]+Coeffs[name][1]*sigMass;
	}
        else 
        {
...
int main(int argc, const char* argv[])
{
  ...
  Bool_t unknowme=false; // Francois Lagarde
  Bool_t useGenerated=false;
  ...
  try
	...
	("useSigTheoryUnc", po::value<bool>(&useSigTheoryUnc)->default_value(false), "option to add an uncertainty to the datacard for the SM diHiggs theory uncertainty. Default is off.")
        ("unknowme", po::value<bool>(&unknowme)->default_value(false), "Unknow reference Masses.")
        ;
      po::variables_map vm;
  ...
  TString dir = TString::Format("/afs/cern.ch/user/m/mgouzevi/public/FOR_LAGARDE/v%d_%s",version,analysisType.c_str());
  ...
  if( !IsKnown(sigMass) ) ssignal="Generated for mass "+to_string(sigMass);

  cout<<"Signal: "<<ssignal<<endl;
  cout<<"Data: "<<ddata<<endl;
  //

  if(IsKnown(sigMass)) AddSigData(w, mass,ssignal);
...
void SigModelFit(RooWorkspace* w, Float_t mass) {
  ...
  for (int c = 0; c < ncat; ++c) {
    ...
    if (IsKnown(sigMass)) SigPdf[c]->fitTo(*sigToFit[c],Range("SigFitRange"),SumW2Error(kTRUE));
    ...
    if (!IsKnown(sigMass))
    {
    		TIterator* iter(w->set(TString::Format("SigPdfParam_cat%d",c))->createIterator());
    		for (TObject *a = iter->Next(); a != 0; a = iter->Next()) 
   		{
    			RooRealVar *rrv = dynamic_cast<RooRealVar *>(a);
    			if (rrv)  rrv->setVal(CalculateValue(rrv->GetName(),sigMass)); 
    		}
    }
...
void MakePlots(RooWorkspace* w, Float_t Mass) {
  ...
  RooPlot* plotmggAll = mgg->frame(Range("SigPlotRange"),Bins(nBinsMass));
   if(IsKnown(sigMass)) signalAll->plotOn(plotmggAll);
  ...
  for (int c = 0; c < ncat; ++c) {
    ...
    if(IsKnown(sigMass))   sigToFit[c]->plotOn(plotmgg[c]);
  ...
  cout << "Plot signal categories" << endl;
  ...
  for (int c = 0; c < ncat; ++c) {
    ...
    if(IsKnown(sigMass)) sigToFit[c]->plotOn(plotmjj[c]);
...
void MakeDataCard(RooWorkspace* w, const char* fileBaseName, const char* fileBkgName , const char* fileHiggsNameggh, const char* fileHiggsNametth, const char* fileHiggsNamevbf, const char* fileHiggsNamevh, const char* fileHiggsNamebbh, Bool_t useSigTheoryUnc) {
  ...
  Float_t siglikeRate[ncat];

  //Francois Lagarde
  
  siglikeRate[0] = (IsKnown(sigMass))?sigToFit[0]->sumEntries():CalculateValue("High",sigMass);
  siglikeRate[1] = (IsKnown(sigMass))?sigToFit[1]->sumEntries():CalculateValue("High_medium",sigMass);

  
  for (int c = 0; c < ncat; ++c) {
    cout << TString::Format("#Events Signal cat%d: ",c) << siglikeRate[c] << endl;
    siglikeErr[c]=0.6*siglikeRate[c];
  }
  cout << "====================================================" << endl;
  ...
  if(addHiggs) { //
    ...
    outFile << "\nrate "
	    <<" "<< siglikeRate[0] <<" "<<1<<" "<<higToFitggh[0]->sumEntries()<<" "<<higToFittth[0]->sumEntries()<<" "<<higToFitvbf[0]->sumEntries()<<" "<<higToFitvh[0]->sumEntries()<< " "<<higToFitbbh[0]->sumEntries()
	    <<" "<< siglikeRate[1] <<" "<<1<<" "<<higToFitggh[1]->sumEntries()<<" "<<higToFittth[1]->sumEntries()<<" "<<higToFitvbf[1]->sumEntries()<<" "<<higToFitvh[1]->sumEntries()<<" "<<higToFitbbh[1]->sumEntries()
	    <<" ";
    if ( NCAT > 2 ){
